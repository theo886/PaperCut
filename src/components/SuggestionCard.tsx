...
const SuggestionCard: React.FC<SuggestionCardProps> = ({ 
  ...
  // Check if the suggestion has been merged into another suggestion
  const hasMergedInto = suggestion.status === 'Merged' && suggestion.activity?.some(act => act.type === 'merge' && act.to);

  // Find the merge activity to get the target suggestion details
  const mergeActivity = suggestion.activity?.find(act => act.type === 'merge' && act.to);
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 ${suggestion.status === 'Merged' ? 'opacity-75' : ''}`}>
      <div className="p-4">
        ...
        {hasMergedInto && mergeActivity && (
          <div className="mt-1 text-xs text-indigo-600 flex items-center">
            <GitMerge size={12} className="mr-1" /> 
            Merged into <span className="font-medium ml-1">{mergeActivity.to}</span>
          </div>
        )}
